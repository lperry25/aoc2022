...
export default function ProductPage({ day }: { day: string }) {
  ...
  return (
    <div>
      <a href={`https://adventofcode.com/2022/day/${day.split('day')[1]}`} target="_">Go to Advent of Code</a><br/>
      <textarea value={value} onChange={(e) =>setValue(e.target.value)}/><br/>
      <p>First result: {data?.result1}</p><br/>
      <p>Second result: {data?.result2}</p><br/>
    </div>
  )
}
